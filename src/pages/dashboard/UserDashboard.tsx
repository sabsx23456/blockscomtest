...
export const UserDashboard = () => {
    ...
    useEffect(() => {
        ...
        // 3. LISTEN FOR TRANSACTIONS (Instant Balance Update & Refund/Payout Notification)
        const txChannel = supabase
            .channel('my_transactions')
            .on('postgres_changes', {
                event: 'INSERT',
                schema: 'public',
                table: 'transactions',
                ...
                if (newTx.type === 'refund') {
                    showToast(`Refund Received: ₱${newTx.amount.toLocaleString()}`, 'info');
                    if (preferences.soundEffects) playTone(600, 100);
                }

                // Win Notification (Payout)
                if (newTx.type === 'payout') {
                    // Logic handled by bet status update usually, but this is a backup for balance
                }
    ...
    */
    ...
    const fetchCurrentMatch = async () => {
        ...
        let activeQuery = supabase
            .from('matches')
            .select('*')
            .in('status', ['open', 'ongoing', 'last_call'])
            ...
            <div className="flex-1 flex flex-col relative bg-black group min-h-[300px] lg:min-h-0">
                ...
                <div className="relative flex-1 bg-black flex items-center justify-center overflow-hidden">
                    <div className='w-full h-full relative'>
                        {displayStreamUrl && (displayStreamUrl.includes('.m3u8') || displayStreamUrl.includes('.mp4')) ? (
                            <>
                                <Player
                                    url={displayStreamUrl}
                                    width="100%"
                                    height="100%"
                                    playing={isPlaying}
                                    loop={true}
                                    ...
                                    config={{
                                        file: {
                                            forceHLS: displayStreamUrl.includes('.m3u8'),
                                        }
                                    }}
                            ...
                            displayStreamUrl ? (
                                <iframe
                                    src={displayStreamUrl.includes('?') ? `${displayStreamUrl}&autoplay=1&muted=1` : `${displayStreamUrl}?autoplay=1&muted=1`}
                                    style={{
                                        position: 'absolute',
                                        top: 0,
                                        left: 0,
                                        ...
                                        height: '100%',
                                        border: 0,
                                        transform: 'scale(1.2)', // Zoom to crop out viewers/UI
